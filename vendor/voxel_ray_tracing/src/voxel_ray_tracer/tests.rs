...
fn range_weight_falls_off_with_the_squared_range() {
    assert_eq!(range_weight(0.0, 400.0), 1.0);
    // A deviation of one voxel halves the update, two voxels leaves a fifth.
    assert!((range_weight(0.01, 100.0) - 0.5).abs() < 1e-6);
    assert!((range_weight(0.01, 200.0) - 0.2).abs() < 1e-6);

    // What counts is deviation measured in voxels, so a finer map trusts the
    // same sensor less.
    let cfg = Config {
        voxel_size: 0.5,
    ...
    assert_eq!(cfg.sigma_voxels_per_m2(), 0.02);
}
...
fn a_far_ray_spares_a_voxel_a_near_ray_would_delete() {
    ...
    let origin = (0.5, 0.5, 0.5);
    // Just under the full unit a near miss subtracts, so the near ray reaches
    // min_health in one frame and the far ray cannot.
    let health = 0.9;
