...
/// Occupancy evidence. Fractional because a hit or a miss carries only as much
/// weight as its range lets it, see `range_weight`. `min_health` and
/// `max_health` stay whole numbers in the config.
...
// What `#[native_config]` expanded to, written out: see the note in lib.rs.
...
pub struct Config {
    ...
    #[validate(range(min = 1))]
    pub max_health: i32,
    /// Quadratic range error of the depth sensor in 1/m: deviation at distance
    /// `d` is `range_error_coeff * d * d`. Updates that far out count only
    /// `range_weight` health. Zero weights every update equally. A D435 is ~0.006.
    #[validate(range(min = 0.0))]
...
/// Health an update `distance_sq` from the sensor is worth. The sensor's range
/// deviation there measures `sigma_voxels_per_m2 * distance_sq` voxels: one
/// voxel of deviation counts half, two counts a fifth. Zero weights all equally.
#[inline]
fn range_weight(sigma_voxels_per_m2: f32, distance_sq: f32) -> f32 {
    let sigma_voxels = sigma_voxels_per_m2 * distance_sq;
    1.0 / (1.0 + sigma_voxels * sigma_voxels)
}

impl Config {
    /// `range_error_coeff` rescaled to voxels, so the per-point path only has to
    /// multiply it by a squared distance to get `range_weight`'s argument.
    fn sigma_voxels_per_m2(&self) -> f32 {
        self.range_error_coeff / self.voxel_size
    }
...
impl VoxelMap {
    ...
    /// health by `weight`. Keeps the healthy-chunk index and every neighbor's
    /// `support` count in sync. Returns whether the voxel was created.
    fn record_hit(
        &mut self,
        key: VoxelKey,
    ...
    /// Apply a clearing miss: drop the voxel's health by `weight`, removing it
    /// once it reaches `min_health`. Keeps the healthy-chunk index and every
    /// neighbor's `support` count in sync. Returns whether the voxel was removed.
...
/// The voxels this batch returned from, each paired with the weight of its
/// nearest return: several points landing in one voxel still move it once, by
/// as much as the best-placed of them earns.
fn live_voxels(
    points: &[(f32, f32, f32)],
    origin: (f32, f32, f32),
    voxel_size: f32,
    sigma_voxels_per_m2: f32,
    ...
    for &(x, y, z) in points {
        ...
        let weight = range_weight(sigma_voxels_per_m2, distance_sq);
...
pub fn update_map(
    ...
    let sigma_voxels_per_m2 = cfg.sigma_voxels_per_m2();
    let hits = live_voxels(points, origin, cfg.voxel_size, sigma_voxels_per_m2);
...
fn find_misses_along_ray(
    ...
    loop {
        ...
        if let Some(c) = map_voxels.get(&(x, y, z)) {
            ...
            // Weighted by how far out this voxel sits, not how long the ray is:
            // a long ray still clears confidently right in front of the sensor.
            let weight = range_weight(sigma_voxels_per_m2, t_enter * t_enter * ray_len_sq);
            misses
                .entry((x, y, z))
                .and_modify(|best| *best = best.max(weight))
                .or_insert(weight);
